...
// Build Stamp: 2026.01.12.0835 - REVERTED UI
...
export default function SupplierPortalPage() {
  ...
  React.useEffect(() => {
    const doLoad = async () => {
      ...
      try {
        console.log(`[SupplierPortal] Handshake for ${appId} at ${N8N_BASE}`);

        // A. Fetch Onboarding Status (Primary Source for New Suppliers)
        ...
        if (statusRes.ok) {
          ...
          if (statusData && statusData.exists) {
            const primaryEmail = statusData.email || statusData.contactEmail || '';
        ...
        // B. Fetch WP Profile (Verified Account Data)
        ...
        if (profileRes.ok) {
          ...
          if (p && (p.email || p.user_email)) {
            const wpEmail = p.email || p.user_email;
            if (wpEmail) setSuEmail(String(wpEmail));
            if (p.display_name || p.displayName) {
              setUserDisplayName(String(p.display_name || p.displayName));
            }
          }
        }
      } catch (e) {
        console.error("[SupplierPortal] Load Error:", e);
      }
    };
    doLoad();
  }, [appId]);
  ...
  return (
    ...
    <Box sx={{
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: section !== 'welcome' ? '240px 1fr 220px' : '240px 1fr' }, gap: 2, maxWidth: 1280, mx: 'auto' }}>
        ...
        <Typography variant="caption" sx={{ display: 'block', color: '#B0BEC5', mt: 3, opacity: 0.5, textAlign: 'center' }}>
          Build: 2026.01.12.0835 | REVERTED UI | Trace: {appId || 'NONE'}
        </Typography>
