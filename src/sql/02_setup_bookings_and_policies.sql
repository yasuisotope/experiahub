-- =================================================================
-- STEP 2: SETUP BOOKINGS, POLICIES & DATA
-- =================================================================
-- Run this script SECOND (after 01_fix_supplier_columns.sql).
...
-- This policy depends on 'application_id' existing in 'suppliers', which Step 1 ensures.
...
USING (
    application_id IN (
        ...
        OR user_id IS NULL -- allows viewing legacy/onboarding data
    )
);

-- 3. Restore/Verify Master Supplier Record
INSERT INTO public.suppliers (application_id, status, business_name) 
VALUES ('SUP-543C66BA', 'active', 'Demo Supplier')
ON CONFLICT DO NOTHING;
-- Note: If ON CONFLICT fails due to missing constraint, we ignore. 
-- The main goal is to ensure the record exists if table was empty.
-- If duplicate application_id exists, that's fine for now.
